...
st.markdown(
    ...
    <style>
        .title {
            ...
            color: #6A1B9A; /* Deep purple to match Pedagogy branding */
        }
        .subtitle {
            font-family: 'Arial', sans-serif;
            color: #AD1457;
...
# Function to process documents and create a combined graph
def load_and_process_documents(portfolio_file, project_file):
    # Process Portfolio Document
    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
        tmp_file.write(portfolio_file.read())
        portfolio_path = tmp_file.name
    portfolio_loader = PyPDFLoader(portfolio_path)
    portfolio_docs = portfolio_loader.load()
    
    # Process Project Document
    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
        tmp_file.write(project_file.read())
        project_path = tmp_file.name
    project_loader = PyPDFLoader(project_path)
    project_docs = project_loader.load()

    # Combine documents
    combined_docs = portfolio_docs + project_docs
    st.session_state['documents'] = combined_docs[:10]  # Limit for testing
...
def main():
    ...
    st.subheader("Upload your Portfolio and Project PDFs to start asking questions")

    # File uploader for the PDFs
    portfolio_file = st.file_uploader("Upload your Portfolio PDF here 👇:", type="pdf", key="portfolio")
    project_file = st.file_uploader("Upload your Project PDF here 👇:", type="pdf", key="project")
    
    if portfolio_file and project_file:
        with st.spinner("Processing..."):
            load_and_process_documents(portfolio_file, project_file)
            st.success("Both documents have been processed and are ready for queries.")
    ...
    if st.session_state['ready']:
        if 'generated' not in st.session_state:
            st.session_state['generated'] = ["Welcome! You can now ask any questions regarding the combined content."]
        ...
        with container:
            ...
            if submit_button and query:
                ...
                # Process the query with GraphRAG
                graph_rag = GraphRAG()
                graph_rag.process_documents(st.session_state['documents'])
                raw_output = graph_rag.query(query)

                # Extract only the answer without metadata or graph traversal details
                output = raw_output.get("content") if isinstance(raw_output, dict) else str(raw_output)
                output_clean = output.split("Final Answer:")[-1].strip() if "Final Answer:" in output else output

                # Add the AI response to the chat history
                st.session_state['chat_history'].append(AIMessage(content=output_clean))

                # Append query and output for display
                st.session_state.past.append(query)
                st.session_state.generated.append(output_clean)
